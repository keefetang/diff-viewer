...
export interface AutosaveOptions {
   ...
   */
  initialContent?: { original: string; modified: string; title: string };
  ...
  /** Called after every successful save with the response metadata. */
...
export function createAutosave(options: AutosaveOptions): AutosaveHandle {
  ...
  async function doSave(): Promise<void> {
    if (halted) return;
    if (pendingId === null || pendingOriginal === null || pendingModified === null) return;
    ...
    const modified = pendingModified;
  ...
  function save(id: string, original: string, modified: string, title: string, editToken: string | null): void {
    if (halted) return;
