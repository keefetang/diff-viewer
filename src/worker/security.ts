 ...
 *   `X-Robots-Tag: noindex`. `no-cache` (not `no-store`) allows conditional
 *   revalidation via ETag/If-None-Match — caches may store the response but
...
export function applySecurityHeaders(response: Response, isApi: boolean, nonce?: string): Response {
  ...
  // style-src: unsafe-inline required by CodeMirror (inline style injection)
  ...
  //   (handles Vite dynamic imports, etc.)
